...
@interface WADeviceInfo ()
    @end

@implementation WADeviceInfo{
...
-(NSString*) model {
    if (!_model) {
        _model =  [WADeviceInfo getPhoneModel];
    }
    return _model;
}
...
-(NSString*) advertiserID {
    if (!_advertiserID) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= (float) 6.0) {
            NSString *advertiserId = [WADeviceInfo getAdvertiserID:5];
            if (advertiserId != nil &&
...
-(NSString*) vendorID {
    if (!_vendorID) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= (float) 6.0) {
            NSString *identifierForVendor = [WADeviceInfo getVendorID:5];
            if (identifierForVendor != nil &&
                ...
                NSLog(@"VendorID old= %@", identifierForVendor);
            }
        }
    }
    NSLog(@"VendorID New = %@", _vendorID);
    return _vendorID;
}
...
+ (NSString*)getAdvertiserID:(int) maxAttempts
    {
        ...
        if (ASIdentifierManager && sharedManager && advertisingIdentifier) {
            ...
            if (identifier == nil && maxAttempts > 0) {
                ...
                return [WADeviceInfo getAdvertiserID:maxAttempts - 1];
            } else {
                return identifier;
            }
        } else {
...
+ (NSString*)getVendorID:(int) maxAttempts
    {
        ...
        if (identifier == nil && maxAttempts > 0) {
            // Try again every 5 seconds
            [NSThread sleepForTimeInterval:5.0];
            return [WADeviceInfo getVendorID:maxAttempts - 1];
        } else {
            return identifier;
        }
    }
...
+ (NSString*)generateUUID
    {
        ...
        NSString *result = [[WAUtils generateUUID] stringByAppendingString:@"R"];
        return result;
    }
